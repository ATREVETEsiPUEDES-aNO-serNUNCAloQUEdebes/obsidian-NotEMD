...
const chronicleCopyByLocale = {
  ...
  ja: {
    heading: "## Kailiクロニクル",
    description: "このCuatro medios periodosクロニクルは [repo-saga](https://github.com/teee32/repo-saga) のyuanesのビジュアルスタイルをGarantiaちつつ、Division de cuatro periodosそのものを repo-saga がGeneracion directaします。",
    refreshedWorkingTree: (date) => `${date} にAhoraのワーキングツリーからUltimas actualizacionesしました。`,
    refreshedRelease: (tag, date) => `リリースタグ \`${tag}\` aけに ${date} にUltimas actualizacionesしました。`,
    latestCommitDate: (date) => `ultimoコミットdia: ${date}。`,
  },
  ko: {
    heading: "## 개발 연대기",
  ...
  zh: {
    heading: "## Cronica del desarrollo",
    description: "Esta edicion trimestral de The Chronicle conserva [repo-saga](https://github.com/teee32/repo-saga) , y las divisiones de cuartos ahora estan representadas directamente por repo-saga Autogenerado。",
    refreshedWorkingTree: (date) => `La ultima vez fue ${date} Actualizar segun el arbol de trabajo actual.。`,
    refreshedRelease: (tag, date) => `El mas reciente ha sido publicado para tag \`${tag}\` Yu ${date} Actualizar。`,
    latestCommitDate: (date) => `Ultima fecha de presentacion：${date}。`,
  },
  zh_Hant: {
    heading: "## Cronica del desarrollo",
    description: "Esta edicion trimestral de The Chronicle conserva [repo-saga](https://github.com/teee32/repo-saga) El estilo visual original y las divisiones de cuartos ahora se representan directamente mediante repo-saga Autogenerado。",
    refreshedWorkingTree: (date) => `La ultima vez fue ${date} Actualizar segun el arbol de trabajo actual.。`,
    refreshedRelease: (tag, date) => `El mas reciente ha sido publicado para tag \`${tag}\` Yu ${date} Actualizar。`,
    latestCommitDate: (date) => `Ultima fecha de presentacion：${date}。`,
  },
